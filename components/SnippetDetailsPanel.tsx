...
export function SnippetDetailsPanel({ snippet, authorUsername, publicSnippetUrl, isDashboard = false, children }: SnippetDetailsPanelProps) {
	...
	const publicPath = `/${authorUsername}/${snippet.id}`
			...
			<div className="mb-8 rounded-3xl border border-border/70 bg-linear-to-br from-background via-background to-muted/30 p-6 shadow-sm sm:p-8">
				<div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
					<div>
						...
						<h1 className="text-3xl font-semibold tracking-tight text-foreground sm:text-4xl">{snippet.title}</h1>
						...
						{snippet.description && (
							<p className="mt-5 max-w-2xl text-[15px] leading-7 text-muted-foreground">{snippet.description}</p>
						)}
					</div>
